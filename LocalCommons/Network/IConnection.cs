...
namespace LocalCommons.Network
{
    ...
    public abstract class IConnection : IDisposable
    {
        ...
        private void M_AsyncSend_Do()
        {
            try
            {
                if (m_PacketQueue.Count > 0)
                {
                    ...
                    m_CurrentChannel.Send(compiled, compiled.Length, SocketFlags.None); //send the package

                    //ushort length = BitConverter.ToUInt16(compiled, 0); // check if there is still a package
                                        //--- Console Hexadecimal 
                                        //output a log of packets to the console
                                        /*StringBuilder*/
                    ...
                    //m_CurrentChannel.Send(data, data.Length, SocketFlags.None); //send the package data for processing
...
#if DEBUG
                    //--- File Hexadecimal
                    //output a log of packages to a file
                    //do not show Pong
                    //if (data[2] != 0x13)
                    //{
                    ...
                    //    }
                    //
                    //    length = BitConverter.ToUInt16(compiled, offset); //check if there is still a package
                    //    offset += 2;
                    //}
                    //#endif
                }
        ...
        /// <summary>
        /// Adds Packet To Queue And After Send It.
        /// the length is not calculated, it is necessary to add yourself before the package
        /// </summary>
        /// <param name="packet"></param>
        ...
        private void ProceedReceiving(SocketAsyncEventArgs e)
        {
            ...
            while (length > 0 && offset < reader.Size)
            {
                ...
                //--- Console Hexadecimal 
                //first, you need to output the package log to the console
                StringBuilder builder = new StringBuilder();
                builder.Append("Recv: ");
                builder.Append(Utility.IntToHex(length));
                builder.Append(" ");
...
#if DEBUG
                //--- File Hexadecimal
                //the output of the log packets to a file
                //do not output Ping
                //if (data[2] != 0x12)
                //{
                //    var fs = new FileStream(path, FileMode.Append);
                ...
                length = reader.ReadLEUInt16(); //check if there is another package
                offset += 2;
                //and only then send for processing
                HandleReceived(data); //we send the package data for processing
            }
            reader.Clear(); //clean the buffer, otherwise the old data is read
            reader = null;
        }
