...
namespace ArcheAge.ArcheAge.Network
{
    public sealed class NP_SCUnitStatePacket_0x0064 : NetPacket
    {
        ...
        public NP_SCUnitStatePacket_0x0064(ClientConnection net) : base(01, 0x0064)
        {
            ...
            WriteItem(0); //ES_NECK
            WriteItem(net.CurrentAccount.Character.Chest); //ES_CHEST Chest (fabric, leather, armor)	23387
            WriteItem(0); //ES_WAIST
            WriteItem(net.CurrentAccount.Character.Legs); //ES_LEGS Greaves (fabric, leather, armor) 23388
            WriteItem(net.CurrentAccount.Character.Gloves); //ES_HANDS
            WriteItem(net.CurrentAccount.Character.Feet); //ES_FEET Shoes (fabric, leather, armor) 23390
            WriteItem(0); //ES_ARMS
            WriteItem(0); //ES_BACK	
            ...
            WriteItem(net.CurrentAccount.Character.Weapon); //ES_MAINHAND Weapon
            WriteItem(net.CurrentAccount.Character.WeaponExtra); //ES_OFFHAND Additional weapon
            WriteItem(net.CurrentAccount.Character.WeaponRanged); //ES_RANGED Bow
            WriteItem(net.CurrentAccount.Character.Instrument); //ES_MUSICAL instrument (string, wind, percussion)
            //}
            //for (int i = 0; i < 7; i++)
            //{
            ...
            switch (net.CurrentAccount.Character.Ext)
            {
                ...
                default:
                    ...
                    string subString = net.CurrentAccount.Character.Modifiers.Substring(0, 256); //it is necessary to cut at the end of two characters \0\0
