...
namespace ArcheAge.ArcheAge.Network
{
    public sealed class NP_SCCharacterStatePacket_0x003B : NetPacket
    {
        private void WriteItem(int itemId)
        {
            ...
            switch (itemId)
            {
                case 0:
                    break;
                default:
                    ns.Write((long)Program.ObjectUid.Next()); //id[1] Q //TODO: make things permanent UID
        ...
        public NP_SCCharacterStatePacket_0x003B(ClientConnection net, uint characterId, byte gm) : base(01, 0x003B)
        {
            ...
            CharacterHolder.LoadEquipPacksData(chr, chr.Ability[0]); //also read NewbieClothPackId, NewbieWeaponPackId table character_equip_packs
            CharacterHolder.LoadClothsData(chr, chr.NewbieClothPackId); //additionally read Head, Chest, Legs,Gloves, Feet from the equip_pack_cloths table
            CharacterHolder.LoadWeaponsData(chr, chr.NewbieWeaponPackId); //also read Weapon,Extra Weapon,Ranged Weapon,Instrument table equip_pack_weapons
            CharacterHolder.LoadCharacterBodyCoord(chr, chr.CharRace, chr.CharGender); //in addition, read body, x, y, z from the character model table
            CharacterHolder.LoadZoneFaction(chr, chr.CharRace, chr.CharGender); //additionally read FactionId, Starting Zone Id from the characters table
            ...
            WriteItem(chr.WeaponRanged); //ES_RANGED Bow
            WriteItem(0); //chr.Instrument); //ES_MUSICAL instrument (string, wind, percussion)
            //}
            ...
            //10-LOVE stalking, 5-DEATH mysticism, 8-VOCATION stealth
            ...
            */
            ...
            ns.Write((short)0x36); //laborPower h  //Labor Points = 5000
            ...
            ns.Write((long)0xC2); //moneyAmount Q  Number of copper coins Automatic 1:100:10000 Convert gold coins  //silver, gold and platinum (origin)
            ...
            ns.Write((long)0x1E); //bmPoint Q  //gift coins = 30
