...
namespace ArcheAge.ArcheAge.Network
{
    public sealed class NP_0x05_CharacterListPacket_0x0079 : NetPacket
    {
        ...
        public NP_0x05_CharacterListPacket_0x0079(ClientConnection net, int num, int last) : base(05, 0x0079)
        {
            ...
            if (totalChars == 0)
            {
                ...
                return; //if the list is empty, finish the job
            }
            else
            {
            ...
            foreach (Character chr in charList)
            {
                if (num == aa) //the NUM parameter corresponds, which Chara to display in the package (can be from 0 to 2)
                {
                    ...
                    CharacterHolder.LoadWeaponsData(chr, chr.NewbieWeaponPackId); //read more прочитать Weapon,WeaponExtra,WeaponRanged,Instrument из таблицы equip_pack_weapons
                    ...
                    //------------------------------------
                    //equip character cycle
                    //------------------------------------
                    //0.helmet (0 head)
                    //1.bib (23387 rough shirt )
                    //2.belt (0 )
                    //3.Bracers (0 )
                    //4.gloves (0 )
                    //5.cloak (0 )
                    //6.greaves (23388 rough pants)
                    //7.shoes (23390 rough shoes)
                    //8. cloak
                    //9. underwear
                    //10.underwear
                    //11.necklace
                    ...
                    //17.additional weapon
                    // / / 18.ranged weapons
                    //19.tool
                    // { 7, times, objects on character?
                    // {1}
    ...
    public sealed class NP_CharacterListPacket_0x0079 : NetPacket
    {
        public NP_CharacterListPacket_0x0079() : base(05, 0x0079)
        {
            ...
            //68000000
            ns.Write((int)0x68);
            //ns.Write((int)0x00); //if 00, it is possible in your faction factionName write
            //size.factionName
            //0000
            //msg = "My superb faction";
            msg = "";
            ns.WriteUTF8Fixed(msg, msg.Length);
            //type 4
            //00000000
            ...
            // { 7, раз, the items on the character?
            // {1}
            //     type 4 ItemID Head
            ns.Write((int)0x5052);
            //     id 8
            ...
            //}
            //laborPower 2
            ns.Write((short)0x1388); //Labor Points= 5000
            ...
            ns.Write((long)0x1E); //silver, gold and platinum (origin)
                                  //moneyAmount 8
            ns.Write((long)0x00); //silver, gold and platinum (continued)
                                  //crimePoint 2
            ns.Write((short)0x00);
            //crimeRecord 4
            ns.Write((int)0x00);
            ...
            ns.Write((int)0x00);
            //bmPoint 8
            ns.Write((long)0x1E); //gift coins = 30
                                  //moneyAmount 8
            ns.Write((long)0x00);
            //moneyAmount 8
            ns.Write((long)0x00);
...
*/
            ...
            // validFlags 4
            ns.Write((int)0x011F8054);
            // { 7, again, the items on the character?
            // {1}
            //     type 4 ItemId Head
            ns.Write((int)0x5103);
            //     id 8
    ...
    public sealed class NP_Packet_CharList_empty_0x0079 : NetPacket
    {
        ...
        public NP_Packet_CharList_empty_0x0079() : base(05, 0x0079)
        {
            ...
            */
            ...
            //00
            ns.Write((byte)0x00);
            //further data were not read from the package!!!
        }
    }
