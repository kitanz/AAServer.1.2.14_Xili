...
namespace ArcheAge.ArcheAge.Network.Packets.Server
{
    public sealed class NP_SCCreateCharacterResponse_0x0033 : NetPacket
    {
        ...
        public NP_SCCreateCharacterResponse_0x0033(ClientConnection net, int num, int last) : base(01, 0x0033)
        {
            ...
            {
                if (num == aa) //the NUM parameter is responsible, which Chara to display in the package (can be from 0 to 2)
                {
                    ...
                    ns.Write((int)179); //starting_zone_id : field in the table characters //zid d
                    ns.Write((int)101); //faction_id : field in the table characters //type d
                    ...
                    WriteItem(0); //suit/costume
                    WriteItem(0); //helmet
                    WriteItem(23387); //ChestPlate
                    WriteItem(0); //belt
                    WriteItem(23388); //leggings
                    WriteItem(0); //bracers
                    WriteItem(23390); //shoes
                    WriteItem(0); //cloak
                    WriteItem(0); //underwear or undergarment
                    WriteItem(0); //underwear or undergarment
                    WriteItem(0); //necklace
                    WriteItem(0); //earring
                    WriteItem(0); //earring
                    WriteItem(0); //ring
                    WriteItem(0); //ring
                    WriteItem(6102); //weapon, right hand
                    WriteItem(0); //additional weapon, left hand
                    WriteItem(0); //WriteItem(6127); //ranged weapons
                    WriteItem(0); //WriteItem(6202); //tool
                    for (int i = 0; i < 7; i++)
                    {
