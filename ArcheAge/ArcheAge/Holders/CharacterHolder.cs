    ...
    public class CharacterHolder
    {
        ...
        public static void DeleteCharacterData(uint characterId)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(Settings.Default.DataBaseConnectionString))
            {
                try
                {
                    conn.Open(); //Establish a connection to the database
        ...
        /// <summary>
        /// Take from the base of the value of the body for the character
        /// </summary>
        /// <param name="chr">where we will write data</param>
        /// <param name="charRace">a character's race</param>
        /// <param name="charGender">the gender of your character</param>
        /// <returns>returns the value of the body for the character</returns>
            ...
            using (MySqlConnection conn = new MySqlConnection(Settings.Default.DataBaseConnectionString))
            {
                ...
                catch (Exception ex)
                {
                    Logger.Trace("Cannot InsertOrUpdate template for " + character.CharName + ": {0}",
                        ex); // ex.Message - write so, if you need only a message without specifying a string in the courses
                }
                finally
                {
                    conn.Close();
