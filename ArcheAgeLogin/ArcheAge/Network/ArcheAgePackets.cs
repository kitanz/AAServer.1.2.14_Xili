...
namespace ArcheAgeLogin.ArcheAge.Network
{
        ...
        public AcAuthResponse_0X03(string clientVersion, ArcheAgeConnection net) : base(0x03, true)
        {
            switch (clientVersion)
            {
                case "1":
                    ...
                    ns.Write((int)net.CurrentAccount.AccountId); // write the AccountID
                    //wsk - wide string key, each session is one and the same, even when relogging (the server selection)
                    string wsk = "A18D7A05E22E459BD1A819222B821030"; //For testing
                ...
                */
                //v.3.0.3.0
                case "3":
                    ns.Write((long)net.CurrentAccount.AccountId); // write the AccountID
                                                                  //wsk - wide string key, each session is one and the same, even when relogging (the server selection)
                    wsk = "A18D7A05E22E459BD1A819222B821030"; //For Testing
                ...
                default:
                    ...
                    ns.Write((long)net.CurrentAccount.AccountId); // write the AccountID
                    //wsk - wide string key, each session is one and the same, even when relogging (the server selection)
                    wsk = "c4099206c8f248c59efdc3f6acffdff7"; //For Testing
                    ns.WriteUTF8Fixed(wsk, wsk.Length);
                    ns.Write((short)0x00);
                    //slotCount
                    ns.Write((byte)0x00);
            ...
            switch (clientVersion)
            {
                ...
                */
                ...
                case "3":
                    ...
                    const string msg = "You are in X2 Auth Server, welcome!"; //For testing
                    ns.WriteUTF8Fixed(msg, msg.Length);
                    break;
                default:
                    ns.WriteHex(
                        "012300596F752061726520696E2058322041757468205365727665722C2077656C636F6D6521"); //write bytes without spaces
                    break;
            }
        }
    }
        ...
        public AcWorldList_0X08(string clientVersion, ArcheAgeConnection net) : base(0x08, true)
        {
            switch (clientVersion)
            {
                case "1":
                    ...
                    int CharCount = CharacterHolder.LoadCharacterData(net.CurrentAccount.AccountId); //are reading data characters and their number of
                    ...
                    if (CharCount != 0)
                    {
                        long m_AccountId = net.CurrentAccount.AccountId; //read only the data of our characters
                        foreach (Character n_Current in CharacterHolder.CharactersList)
                        {
                            if (n_Current.AccountId == m_AccountId)
                            {
                                ...
                                string uid = n_Current.GUID; // = ""; //UID - parameter options, possibly an empty string!
                    ...
                    break;
                case "3":
                    //4E000800  //trial recording with one remote enchantment-gnome
                    ...
                    //1D000800  //trial recording server Archeage, no of chars the beginning of the creation
                    ////ns.WriteHex("010101000900417263686552616765010000000000000000000000");
                    //v.3.0.3.0
                    //Send a list of servers, number of chars on the accounts
                    ...
                    {
                        ns.Write((byte)server.Id);
                        ns.Write((byte)0x01); //the inscription in the server list 00-no inscriptions, 01 - NEW, 02-UNITED 03-UNITED 04-no inscriptions
                        ns.Write((byte)0x02); //the color of the inscription in the server list 00-blue, 01-green, 02-violet, 03, 04, 08-red, 0x10-
                        ns.WriteUTF8Fixed(server.Name, Encoding.UTF8.GetByteCount(server.Name));
                    ...
                    CharCount = CharacterHolder.LoadCharacterData(net.CurrentAccount.AccountId); // are reading data characters and their number of
                    ...
                    if (CharCount != 0)
                    {
                        ...
                        foreach (Character n_Current in CharacterHolder.CharactersList)
                        {
                            if (n_Current.AccountId == m_AccountId)
                            {
                                ...
                                string charname = n_Current.CharName;
                                ns.WriteASCIIFixed(charname, charname.Length);
                                ns.Write((byte)n_Current.CharRace); //Char Race - 01=nuian, 03 = dwarves
                                ns.Write((byte)n_Current.CharGender); //CharGender - 01-М, 02=Female
                                string uid = n_Current.GUID; //UID - Parameters enchantment
                                ns.WriteHex(uid, uid.Length);
                                ns.Write((long)n_Current.V); //v
                            }
                        }
    ...
    public sealed class NP_ACLoginDenied_0x0C : NetPacket
    {
        public NP_ACLoginDenied_0x0C() : base(0x0C, true)
        {
             ...
             */
            //v.3.0.3.0
            ns.Write((byte)0x02);  //Reason "there is no such user"
            ns.Write((short)0x00); //Undefined
            ns.Write((short)0x00); //Undefined
        }
    }
